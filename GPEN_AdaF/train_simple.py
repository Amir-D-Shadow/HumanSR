...
import __init_paths
from training.data_loader.dataset_face import FaceDataset, HQFaceDataset
...
def validation(model, HQ_img, lpips_func, args, device):
...
def train(args, loader, ref_loader, generator, discriminator, losses, g_optim, d_optim, g_ema, lpips_func, device):
    ...
    for idx in pbar:
        ...
        HQ_img = next(ref_loader)
        B,C,H,W = HQ_img.shape
        HQ_img = HQ_img.view(-1,H,W).unsqueeze(0)
        HQ_img = HQ_img.to(device)

        requires_grad(generator, False)
        requires_grad(discriminator, True)

        fake_img, _ = generator(degraded_img,HQ_img)
        ...
        if g_regularize:
            ...
            fake_img, latents = generator(degraded_img,HQ_img, return_latents=True)
        ...
        if get_rank() == 0:
            ...
            if i % args.save_freq == 0:
                with torch.no_grad():
                    g_ema.eval()
                    sample, _ = g_ema(degraded_img,HQ_img)
                ...
                lpips_value = validation(g_ema,HQ_img, lpips_func, args, device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ref_path', type=str, default='ref')
    ...
    hq_dataset = HQFaceDataset(args.ref_path, args.size)
    ref_loader = data.DataLoader(
        hq_dataset,
        batch_size=10,
        sampler=data_sampler(hq_dataset, shuffle=True, distributed=args.distributed),
        drop_last=True,
    )

    train(args, loader, ref_loader, generator, discriminator, [smooth_l1_loss, id_loss], g_optim, d_optim, g_ema, lpips_func, device)
