...
class FullGenerator(nn.Module):
    def __init__(
        ...
        self.initial_proj1 = ConvLayer(30, channels[size], 1, device=device)
        ...
        for i in range(self.log_size, 2, -1):
            out_channel = channels[2 ** (i - 1)]
            #conv = [ResBlock(in_channel, out_channel, blur_kernel)]
            self.Ref_img_filtering.append( AdaConv(in_channel,out_channel,3,style_dim,downsample=True,blur_kernel=blur_kernel,isconcat=isconcat,device=device) )
    ...
    def forward(self,
        ...
        exStyle = exStyle.unsqueeze(1).repeat(1, n_latent, 1)

        hq_noise = []
        y = self.initial_proj1(hq_x)
        y = self.initial_proj2(y,exStyle[:,0],noise=noise[0])
        y = self.initial_proj3(y,exStyle[:,1],noise=noise[1])
        
        hq_noise.append(y)
        i = 1
        for hq_Conv1, hq_Conv2, noise1, noise2 in zip(self.Ref_img_filtering[::2], self.Ref_img_filtering[1::2], noise[2::2], noise[3::2]):
            y = hq_Conv1(y,exStyle[:,i],noise=noise1)
            y = hq_Conv2(y,exStyle[:,i+1],noise=noise2)
            hq_noise.append(y)
            i = i + 2

        y = y.view(y.shape[0], -1)
        HQouts = self.final_linear_HQ(y)
