...
def validation(model, ref_img, lpips_func, args, device):
    ...
    for lq_f, hq_f in zip(lq_files, hq_files):
        ...
        with torch.no_grad():
            img_out, __ = model(img_t,ref_img)
...
def train(args, loader, ref_loader, generator, discriminator, losses, g_optim, d_optim, g_ema, lpips_func, device):
    ...
    for idx in pbar:
        ...
        _, ref_img = next(ref_loader)
        B,C,H,W = ref_img.shape
        ref_img = ref_img.view(-1,H,W).unsqueeze(0)
        ...
        fake_img, _ = generator(degraded_img,ref_img)
        ...
        if g_regularize:
            ...
            fake_img, latents = generator(degraded_img, ref_img, return_latents=True)
        ...
        if get_rank() == 0:
            ...
            if i % args.save_freq == 0:
                with torch.no_grad():
                    g_ema.eval()
                    sample, _ = g_ema(degraded_img,ref_img)
                ...
                lpips_value = validation(g_ema, ref_img, lpips_func, args, device)
...
if __name__ == '__main__':
    ...
    ref_data = FaceDataset(args.ref_dir,args.size)
    ref_loader = data.DataLoader(
        ref_data,
        batch_size=10,
        sampler=data_sampler(ref_data, shuffle=True, distributed=args.distributed),
        drop_last=True,
    )
