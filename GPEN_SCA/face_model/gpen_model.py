...
class Generator(nn.Module):
    ...
    def forward(
        ...
        if len(styles) < 2:
            ...
            """
            c_dim_mid = styles[0].shape[-1] // 2 
            latent = torch.cat([styles[0][:,:c_dim_mid],styles[1][:,:c_dim_mid]],dim=-1).unsqueeze(1).repeat(1, inject_index, 1)
            latent2 = torch.cat([styles[0][:,c_dim_mid:],styles[1][:,c_dim_mid:]],dim=-1).unsqueeze(1).repeat(1, self.n_latent - inject_index, 1)
...
class FullGenerator(nn.Module):
    def __init__(
        ...
        self.projl = ConvLayer(3, channels[size], 1, device=device)
        self.projr = nn.Sequential(EncLinear(in_dim=30, out_dim=channels[size], bias=True, bias_init=0, lr_mul=0.01, activation="fused_lrelu", device=device),
                                   PixelNorm(),
                                   EncLinear(in_dim=channels[size], out_dim=channels[size], bias=True, bias_init=0, lr_mul=0.01, activation="fused_lrelu", device=device),
                                   EncLinear(in_dim=channels[size], out_dim=channels[size], bias=True, bias_init=0, lr_mul=0.01, activation="fused_lrelu", device=device),
        )
        conv = []
        ...
        for i in range(self.log_size, 2, -1):
            ...
            conv = [Encoder_Block(in_channel, out_channel, 3, downsample=True, device=device)] 
            setattr(self, self.names[self.log_size-i+1], MySequential(*conv))
            in_channel = out_channel
        self.final_linearL = nn.Sequential(EqualLinear(channels[4] * 4 * 4, style_dim, activation='fused_lrelu', device=device))
        self.final_linearR = nn.Sequential(EqualLinear(channels[4] * 4 * 4, style_dim, activation='fused_lrelu', device=device))
    ...
    def forward(self,
        ...
        noise = []
        yl = self.projl(xl)
        yr = xr.permute(0, 2, 3, 1).contiguous()   #(B,H,W,C)
        yr = self.projr(yr)                        #(B,H,W,C)
        yr = yr.permute(0, 3, 1, 2).contiguous()   #(B,C,H,W)

        midIndex = yl.shape[1] // 2
        mixedlow = torch.cat([yl[:,:midIndex,:,:],yr[:,:midIndex,:,:]],dim=1)
        mixedhigh = torch.cat([yl[:,midIndex:,:,:],yr[:,midIndex:,:,:]],dim=1)
        noise.append((mixedlow,mixedhigh))
        ...
        outsL = self.final_linearL(yl)
        yr = yr.view(yr.shape[0],-1)
        outsR = self.final_linearR(yr)
        #print(outs.shape)
        #noise = list(itertools.chain.from_iterable(itertools.repeat(x, 2) for x in noise))[::-1]
        noise = list(itertools.chain.from_iterable([ml,mr] for ml,mr in noise))[::-1]
        outs = self.generator([outsL,outsR], return_latents, inject_index, truncation, truncation_latent, input_is_latent, noise=noise[1:])
        return outs
...
class EncLinear(nn.Module):
    def __init__(
        ...
        if bias:
            self.bias = nn.Parameter(torch.zeros(1,1,1,out_dim).fill_(bias_init))
...
class Encoder_Block(nn.Module):
    ...
    def __init__(self,in_channel, out_channel, kernel_size, downsample=True, device="cpu"):

        super().__init__()
        
        self.downl = ConvLayer(in_channel=in_channel, out_channel=out_channel, kernel_size=kernel_size, downsample=downsample, device=device)

        self.downr = ConvLayer(in_channel=in_channel, out_channel=out_channel, kernel_size=kernel_size, downsample=downsample, device=device)
        """
        self.normr = PixelNorm()
        self.projr = nn.Sequential(EncLinear(in_dim=out_channel, out_dim=out_channel, bias=True, bias_init=0, lr_mul=0.01, activation="fused_lrelu", device=device),
                                   EncLinear(in_dim=out_channel, out_dim=out_channel, bias=True, bias_init=0, lr_mul=0.01, activation="fused_lrelu", device=device),
                                   EncLinear(in_dim=out_channel, out_dim=out_channel, bias=True, bias_init=0, lr_mul=0.01, activation="fused_lrelu", device=device),
                                   EncLinear(in_dim=out_channel, out_dim=out_channel, bias=True, bias_init=0, lr_mul=0.01, activation="fused_lrelu", device=device),
                                   )
        """
        self.cross_attn = SCAM(c=out_channel)
      

    def forward(self,x_l,x_r):
       
        y_l = self.downl(x_l) #(B,C,H,W)

        y_r = self.downr(x_r) #(B,C,H,W)
        """
        y_r = self.normr(y_r) #(B,C,H,W)
        y_r = y_r.permute(0, 2, 3, 1).contiguous()   #(B,H,W,C)
        y_r = self.projr(y_r)     #(B,H,W,C)
        y_r = y_r.permute(0, 3, 1, 2).contiguous()#(B,C,H,W)
        """
        #yOutl,yOutr = y_l,y_r
        yOutl,yOutr = self.cross_attn(y_l,y_r) #(B,C,H,W),(B,C,H,W)
...
if __name__ == "__main__":
    ...
    #net = EqualLinear(in_dim=64, out_dim=128, bias=True, bias_init=0, lr_mul=1, activation=None, device='cpu')
    x_l = torch.randn(6,512,16,16)
    x_r = torch.randn(1,512,16,16)
    net = nn.Sequential(Encoder_Block(in_channel=512, out_channel=1024, kernel_size=3, downsample=True, device="cpu"))

    yl, yr = net(x_l,x_r)
    print(f"y_l: {yl.shape}, y_r: {yr.shape}")