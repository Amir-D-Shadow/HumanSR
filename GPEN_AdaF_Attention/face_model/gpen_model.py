...
class FullGenerator(nn.Module):
    def __init__(
        ...
        self.lq_style_proj = nn.Sequential(*layers)
        self.initial_proj1 = AdaConv(3, channels[size], 1, style_dim, blur_kernel=blur_kernel, isconcat=False, device=device)
        #self.initial_proj1 = nn.Sequential( ConvLayer(3, channels[size], 1, device=device) )
        #self.initial_proj2 = AdaConv(channels[size], channels[size], 3, style_dim, blur_kernel=blur_kernel, isconcat=isconcat, device=device)
        #self.initial_proj3 = AdaConv(channels[size]*feat_multiplier, channels[size], 1, style_dim, blur_kernel=blur_kernel, isconcat=False, device=device)

        self.Ref_img_filtering = nn.ModuleList()
        in_channel = channels[size]
        
        for i in range(self.log_size, 2, -1):
            out_channel = channels[2 ** (i - 1)]
            #conv = [ResBlock(in_channel, out_channel, blur_kernel)]
            self.Ref_img_filtering.append( AdaConv(in_channel,out_channel,3,style_dim,downsample=True,blur_kernel=blur_kernel,isconcat=False,device=device) )
            #self.Ref_img_filtering.append( AdaConv(out_channel*feat_multiplier,out_channel,1,style_dim,downsample=False,blur_kernel=blur_kernel,isconcat=False,device=device) )
            in_channel = out_channel

        #attention layers
        self.q_proj = nn.Sequential(EqualLinear(channels[4] * 4 * 4 , channels[4] * 4 * 4, activation='fused_lrelu', device=device))
        self.k_proj = nn.Sequential(EqualLinear(channels[4] * 4 * 4 , channels[4] * 4 * 4, activation='fused_lrelu', device=device))
        self.v_proj = nn.Sequential(EqualLinear(channels[4] * 4 * 4 , channels[4] * 4 * 4, activation='fused_lrelu', device=device))

        self.atten_scale = math.sqrt( channels[4] * 4 * 4 )
        self.final_linear_HQ = nn.Sequential(EqualLinear(channels[4] * 4 * 4 , style_dim, activation='fused_lrelu', device=device))
    ...
    def forward(self,
        ...
        #HQ process 
        hq_batch = hq_x.shape[0]
        n_latent = self.log_size - 1
        exStyle = self.lq_style_proj(LQouts)
        exStyle = exStyle.unsqueeze(1).repeat(hq_batch, n_latent, 1)

        hq_noise = []
        y = self.initial_proj1(hq_x,exStyle[:,0],noise=noise[0])
        #y = self.initial_proj2(y,exStyle[:,0],noise=noise[0])
        #y = self.initial_proj3(y,exStyle[:,1],noise=noise[1])
        
        hq_noise.append(y.mean(dim=0,keepdim=True))
        i = 0
        for hq_Conv1, noise1 in zip(self.Ref_img_filtering, noise[1:]):
            y = hq_Conv1(y,exStyle[:,i],noise=noise1)
            hq_noise.append(y.mean(dim=0,keepdim=True))
            i = i + 1
        ...
        yq = self.q_proj(y)
        yk = self.k_proj(y).permute(1,0).contiguous()
        yv = self.v_proj(y)

        atten_energy = torch.matmul( F.softmax( torch.matmul(yq,yk) / self.atten_scale , dim = -1) , yv )
        atten_energy = atten_energy.mean(dim=0,keepdim=True)

        HQouts = self.final_linear_HQ(atten_energy)
        hq_noise = list(itertools.chain.from_iterable(itertools.repeat(x, 2) for x in hq_noise))[::-1]
        
        C_dim = HQouts.shape[-1]
        yl = torch.cat([LQouts[:,:C_dim//2],HQouts[:,:C_dim//2]],dim=-1)
        yr = torch.cat([LQouts[:,C_dim//2:],HQouts[:,C_dim//2:]],dim=-1)

        outs = self.generator([yl,yr], return_latents, inject_index, truncation, truncation_latent, input_is_latent, noise=hq_noise[1:])
